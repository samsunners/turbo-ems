...
byte setError(byte errorID)
{
    if(errorCount < MAX_ERRORS)
    {
        ...
        errorCount++;
        if(errorCount == 1) {
            BIT_SET(currentStatus.spark, BIT_SPARK_ERROR);    //Enable the error indicator
        }
    }
    return errorCount;
}
...
void clearError(byte errorID)
{
    ...
    if (errorID == errorCodes[0]) {
        clearedError = 0;
    }
    else if(errorID == errorCodes[1]) {
        clearedError = 1;
    }
    else if(errorID == errorCodes[2]) {
        clearedError = 2;
    }
    else if(errorID == errorCodes[3]) {
        clearedError = 3;
    }
    ...
    if(clearedError < MAX_ERRORS)
    {
        ...
        errorCount--;
        if(errorCount == 0) {
            BIT_CLEAR(currentStatus.spark, BIT_SPARK_ERROR);    //Enable the error indicator
        }
    }
}
